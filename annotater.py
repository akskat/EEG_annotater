#!/usr/bin/env python3
"""
new_annotater.py  (MODIFIED – spiller tydelig lyd ved hvert kategoriskift)
...
Et Tkinter-basert GUI som:
  ...
  - Viser en sekvens med 30 kategorier (hver 10 sek), med nedtelling.
  - Kaller BrainVision Recorder via OLE i main-tråden for å unngå
    «marshalled for a different thread»-feil.
  - **NYTT:** Spiller en tydelig dobbel-pip (1200 Hz → 1500 Hz) hver gang
    kategorien skifter, slik at brukeren hører overgangen.
"""
...
from tkinter import ttk, filedialog   # GUI + mappe-dialog
import pandas as pd

# COM / BrainVision Recorder
import pythoncom
import win32com.client

# ──────────────────────────────────────────────────────────────────────
# Lyd: platform-uavhengig «dobbel-pip»
# ──────────────────────────────────────────────────────────────────────
try:
    # Windows – bruk innebygget winsound
    import winsound

    def play_sound() -> None:
        """Spill to korte pip (1200 Hz + 1500 Hz) på Windows."""
        winsound.Beep(1200, 200)   # 200 ms
        winsound.Beep(1500, 150)   # 150 ms

except ImportError:
    # macOS/Linux – fall tilbake på simpleaudio + numpy
    try:
        import numpy as np
        import simpleaudio as sa

        _FS = 44_100  # sampling-frekvens (Hz)

        def _tone(freq: int, duration: float, volume: float = 0.8) -> np.ndarray:
            t = np.linspace(0, duration, int(_FS * duration), False)
            wave = np.sin(2 * np.pi * freq * t) * volume
            return (wave * 32767).astype(np.int16)

        def play_sound() -> None:
            """Dobbel-pip for macOS/Linux (simpleaudio)."""
            audio = np.concatenate([_tone(1200, 0.20), _tone(1500, 0.15)])
            sa.play_buffer(audio, 1, 2, _FS).wait_done()

    except ImportError:
        # Ingen lydbibliotek tilgjengelig → stillhet
        def play_sound() -> None:
            pass
...
CATEGORY_DURATION = 30         # sekunder pr. kategori
DELAY_BEFORE_START = 10        # sekunder før sekvensen starter

DEFAULT_EEG_FOLDER = (
    r"C:\Users\vislab\Documents\Master Aksel\EEG_master\EEG_dataset"
)
...
class RecordingUI:
    def __init__(self, root):
        ...
        style.configure(
            "Header.TLabel", font=("Helvetica", 24, "bold"), background="#FFFFFF"
        )
        style.configure(
            "Big.TLabel", font=("Helvetica", 56, "bold"), background="#E0FFE0"
        )
        style.configure(
            "Medium.TLabel", font=("Helvetica", 22), background="#FFFFD0"
        )
        style.configure(
            "Small.TLabel", font=("Helvetica", 16), background="#E0E0FF"
        )
        style.configure("TButton", font=("Helvetica", 14), padding=10)
        style.configure(
            "Browse.TButton",
            font=("Helvetica", 10),  # mindre font
            padding=(5, 2),          # mindre padding (H, V)
        )
        ...
        # OLE: Opprett Recorder-objekt i main-tråd
        self.recorder = None
        self.init_recorder()

        # Les Excel-fil med kategorier
        self.df = pd.read_excel(EXCEL_PATH)
        self.recording_cols = self.df.columns[1:]  # kol. 0 = «Category»
        ...
        self.title_label = ttk.Label(
            top_frame,
            text="EEG Category Playback (OLE)",
            style="Header.TLabel",
        )
        ...
        folder_entry = ttk.Entry(
            storage_frame, textvariable=self.folder_path_var, width=45
        )
        ...
        filename_entry = ttk.Entry(
            storage_frame, textvariable=self.filename_var, width=45
        )
        ...
        ttk.Label(rec_frame, text="Velg Recording:").pack(
            side="left", padx=5
        )
        ...
        self.recording_combobox = ttk.Combobox(
            ...
            values=list(self.recording_cols),
            width=20,
            state="readonly",
        )
        ...
        self.next_cat_label = tk.Label(
            main_frame,
            text="Next category: None",
            font=("Helvetica", 16),
            bg="#E0E0FF",
            highlightbackground="#A0A0D0",
            highlightthickness=2,
        )
    ...
    def init_recorder(self):
        """Opprett COM-objekt i main-tråd slik at OLE-kall fungerer."""
        try:
    ...
    def start_recording(self):
        ...
        # Bakgrunnstråd for selve sekvensen
    ...
    def build_categories(self, col_name):
        """Bygg liste (key_code, tekst) fra valgt kolonne i Excel."""
        cat_list = []
        for i in range(30):
    ...
    # ── Sekvens-kjøring (bakgrunnstråd) ------------------------------
    def run_sequence(self):
        """Bakgrunnstråd: spiller av kategorisekvensen (30 s pr. kategori)."""
        for idx, (key_code, cat_text) in enumerate(self.categories_seq):
            if not self.is_running:
                break
            # Oppdater GUI + Recorder i main-tråden:
            self.root.after(
                0, self.schedule_marker_and_gui, cat_text, idx
            )
    ...
    # ── Utføres i main-tråden (via root.after) ----------------------
    def schedule_marker_and_gui(self, cat_text, idx):
        """Oppdater GUI, send Recorder-markør, **spill lyd**."""
        # Spill pip *før* GUI-oppdatering så lyden matcher skiftet
        play_sound()

        self.update_current_category(cat_text)
        next_cat = (
            self.categories_seq[idx + 1][1]
